class NoteApp {
    ...
    async createRoom() {
        ...
        try {
            const code = await window.p2pManager.createRoom();
            this.roomCode.textContent = code;
            this.roomInfo.style.display = 'block';
            this.connectionStatus.textContent = '等待连接...';
            
            // 初始化同步管理器
            window.syncManager = new SyncManager(window.p2pManager, this);
            
            // 监听连接状态
            window.p2pManager.onConnectionStateChange = (state) => {
                this.connectionStatus.textContent = state;
                if (state === 'connected') {
    ...
    async joinRoom() {
        ...
        try {
            await window.p2pManager.joinRoom(code);
